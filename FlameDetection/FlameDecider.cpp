...
#ifdef TRAIN_MODE
const string FlameDecider::SAMPLE_FILE("/Users/Jaden/flame-detection-system/samples/sample6.txt");
#endif
...
//        label.at<float>(i, 0) = mResultVec[i] ? 1.0 : 0.0;
          label.at<float>(i, 0) = mResultVec[i] ? 1 : 0;
	}
    ...
    */
    ...
    mSVM->setTermCriteria(criteria);
    
//    mSVM.train(data, label, Mat(), Mat(), params);

    Ptr<ml::TrainData> tData = ml::TrainData::create(data, ml::ROW_SAMPLE, label);
    mSVM->train(tData);
...
bool FlameDecider::judge(map<int, Target>& targets)
{
    ...
    namedWindow("result");
//    moveWindow("result", 10, 400);
        moveWindow("result", 640 , 120);
